...
guards_asleep.each do | guard, _count |
  graph.each_with_index do |line, i|
    if graph_test[i][0][2] == guard
  ...
  min_counter.each_with_index do | value, minute |
    if value > most_aslept_value
      most_aslept_value = value
      guard_id = guard
      minute_id = minute
    end
  end
...
puts "Minute #{minute_id}"
puts "Guard #{guard_id}"
puts "Result #{minute_id * guard_id}"
